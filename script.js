...
branches.push({x:stage.w/2,y:stage.h,act:true,l:0,tl:stage.h/2-100,a:Math.PI,s:0,w:15});
let timer = 0;
function enginestep() {
	ctx.clearRect(0,0,stage.w,stage.h);
	timer++;
	if (timer>300) {
		branches = [{x:stage.w/2,y:stage.h,act:true,l:0,tl:stage.h/2-100,a:Math.PI,s:0,w:15}];
		leaves = [];
		apples = [];
		timer = 0;
	}
	...
	branches.forEach(b=>{
		if (b.s<5) {
			...
			if (b.l<b.tl-3) {
				...
				if (b.s==4) {
					if (Math.random()*30<1) {
						apples.push({x:b.x+Math.sin(b.a)*b.l,y:b.y+Math.cos(b.a)*b.l,w:0,sz:Math.random()*5+8});
					} else {
						leaves.push({x:b.x+Math.sin(b.a)*b.l,y:b.y+Math.cos(b.a)*b.l,w:0,sz:Math.random()*5+8,h:0,a:Math.PI*1.5-b.a})
					}
					
				} else {
					for (let i = 0; i < 5; i++) {
						branches.push({x:b.x+Math.sin(b.a)*b.l,y:b.y+Math.cos(b.a)*b.l,act:true,l:0,tl:Math.random()*(150-b.s*35)+20,a:b.a+Math.random()*Math.PI-Math.PI/2,s:b.s+1,w:b.w*0.5});
					}
				}
